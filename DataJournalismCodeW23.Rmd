...
```{r}
df <- df %>% select(LOC_X, LOC_Y)


kmeans.re <- kmeans(df, centers = 6, nstart = 200)
kmeans.re
...
kmeans.re$centers
kmeans.re$centers[, c("LOC_X", "LOC_Y")]
  
# cex is font size, pch is symbol
points(kmeans.re$centers[, c("LOC_X", "LOC_Y")], 
       col = 1:6, pch = 8, cex = 3) 
  
## Visualizing clusters
y_kmeans <- kmeans.re$cluster
clusplot(df[, c("LOC_X", "LOC_Y")],
         y_kmeans,
         lines = 0,
         shade = TRUE,
         color = TRUE,
         ...
         plotchar = FALSE,
         span = TRUE,
         main = paste("Jordan Cluster Chart"),
         xlab = 'X Loc',
         ylab = 'Y Loc')
